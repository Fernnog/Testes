...
async function handleForceSync() {
    ...
    // Melhoria Prioridade 2: Usar o indicador de status global para feedback de UX.
    ...
    allTargetsToSync.forEach(t => {
        ...
        delete t.driveErrorMessage; // Limpa erros antigos
    });
    ...
    try {
        ...
        // Este catch é um fallback, pois syncTarget já lida com erros individuais
        ...
        // Melhoria Prioridade 2: Após tudo, verifica o status final e atualiza a UI global.
        const hasErrors = allTargetsToSync.some(t => t.driveStatus === 'error');
        if (hasErrors) {
    ...
    try {
        ...
        if (user && accessToken) {
            ...
            if (initialized) {
                ...
                // A chamada para loadDataForUser já terá sido feita pelo listener do onAuthStateChanged.
                // Apenas iniciamos a sincronização se os dados já estiverem no estado.
                if (state.prayerTargets.length > 0) {
...
// CORREÇÃO: Função loadDataForUser com lógica de inicialização em segundo plano.
async function loadDataForUser(user) {
    try {
        // Passo 1: Carrega todos os dados do Firestore primeiro.
        ...
        // Passo 2: Atualiza a UI com os dados do Firestore.
        // O usuário já pode usar o app neste ponto.
        applyFiltersAndRender('mainPanel');
        applyFiltersAndRender('archivedPanel');
        applyFiltersAndRender('resolvedPanel');
        ...
        // Passo 3: Tenta inicializar o Drive e a sincronização em segundo plano.
        const isGoogleUser = user.providerData.some(p => p.providerId === 'google.com');
        if (isGoogleUser) {
            try {
                UI.updateDriveStatusUI('syncing', 'Conectando...');
                const { accessToken } = await Auth.signInWithGoogle(); 
                if (accessToken) {
                    const initialized = await GoogleDriveService.initializeDriveService(accessToken);
                    if (initialized) {
                        state.isDriveEnabled = true;
                        UI.updateDriveStatusUI('connected');
                        // Inicia a sincronização em massa apenas após a inicialização bem-sucedida.
                        const allTargetsToSync = [...state.prayerTargets, ...state.archivedTargets, ...state.resolvedTargets];
                        for (const target of allTargetsToSync) {
                            await new Promise(resolve => setTimeout(resolve, 200)); 
                            await syncTarget(target.id);
                ...
                console.warn("Não foi possível inicializar o Drive automaticamente:", error.message);
                UI.updateDriveStatusUI('error', 'Reconecte para sincronizar');
                state.isDriveEnabled = false;
            }
        }
...
// CORREÇÃO: Lógica de inicialização do app.
document.addEventListener('DOMContentLoaded', () => {
    Auth.initializeAuth(async (user) => {
        if (user) {
            // A UI de autenticado é mostrada primeiro, sem esperar o Drive.
            UI.updateAuthUI(user);
            // A lógica de conexão com o Drive é movida para DENTRO de loadDataForUser,
            // para garantir que só aconteça após a verificação do usuário.
    ...
    document.body.addEventListener('click', async e => {
        // CORREÇÃO: Adicionada a lógica de logout na delegação de eventos
        if (e.target.id === 'btnLogoutTop') {
            Auth.handleSignOut();
            return; 
        }
