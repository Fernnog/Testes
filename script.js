...
const firebaseConfig = {
  apiKey: "AIzaSyC4xXSGw91MPLbC3ikCsdJ4pkNu1GZTqKQ", // Mantenha a chave da versão atual
  authDomain: "teste-da-perola.firebaseapp.com",     // Mantenha o authDomain da versão atual
  projectId: "teste-da-perola",                     // Mantenha o projectId da versão atual
  storageBucket: "teste-da-perola.firebasestorage.app", // Mantenha o storageBucket da versão atual
  messagingSenderId: "845747978306",               // Mantenha o messagingSenderId da versão atual
  appId: "1:845747978306:web:90314c25caf38106bc6ddb", // Mantenha o appId da versão atual
  measurementId: "G-BLJ0S9GZLE"                   // Mantenha o measurementId da versão atual
};
...
async function gerarOrcamento() {
    ...
    produtosRows.forEach(row => {
        ...
        if (quantidade > 0 && descricao) { // Adiciona apenas se tiver quantidade e descrição
            ...
            // Opcional: Adicionar alerta para o usuário
            // alert(`Produto na linha ${row.rowIndex + 1} tem quantidade mas falta descrição.`);
        }
    });
    ...
    if (orcamentoId) {
        ...
        exibirChecklistEmHTML(orcamento); // Mostra o checklist
    }
}
            ...
            if (target.tagName === 'BUTTON') {
                ...
                if (target.classList.contains('btnVisualizarOrcamento')) {
                    ...
                    if (orcamento) {
                        ...
                        // exibirChecklistEmHTML(orcamento); // Descomente se quiser checklist ao visualizar
                    } else {
                        console.error("Orçamento não encontrado para visualizar:", orcamentoId);
...
// Idealmente, remova todos os `onclick` do HTML e use apenas `addEventListener`.
// Se precisar manter algum `onclick` por um motivo específico:
// window.filtrarOrcamentos = filtrarOrcamentos; // Exemplo, ID foi adicionado, então não precisa mais
// window.filtrarPedidos = filtrarPedidos; // Exemplo, ID foi adicionado, então não precisa mais
// window.filtrarPedidosRelatorio = filtrarPedidosRelatorio; // Exemplo, ID foi adicionado, então não precisa mais
// window.gerarRelatorioXLSX = gerarRelatorioXLSX; // Exemplo, ID foi adicionado, então não precisa mais