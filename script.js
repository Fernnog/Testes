...
const firebaseConfig = {
  ...
  measurementId: "G-BLJ0S9GZLE"                   // SEU MEASUREMENT ID (se usar Analytics)
};
...
async function gerarOrcamento() {
    ...
    if (orcamentoId) {
        ...
        // Limpa campos textarea explicitamente (reset pode não funcionar 100%)
        const prazoEntregaField = document.getElementById("prazoEntrega");
        if (prazoEntregaField) prazoEntregaField.value = '';
        const observacoesField = document.getElementById("observacoes");
        if (observacoesField) observacoesField.value = '';
        ...
        exibirOrcamentoEmHTML(orcamento); // Mostra a pré-visualização do orçamento
        exibirChecklistEmHTML(orcamento); // <<< EXIBE CHECKLIST
    }
}
...
function exibirOrcamentoEmHTML(orcamento) {
    ...
    janelaOrcamento.onload = () => { // Usa onload em vez de addEventListener('load')
        ...
        const prazoEntregaFormatado = orcamento.prazoEntrega ? `<pre style="white-space: pre-wrap; font-family: inherit; margin: 0;">${orcamento.prazoEntrega}</pre>` : 'Não informado';
        // Formata o campo observacoes para exibição, mantendo quebras de linha
        const observacoesFormatado = orcamento.observacoes ? `<pre style="white-space: pre-wrap; font-family: inherit; margin: 0;">${orcamento.observacoes}</pre>` : '';
...
// Exibir Checklist de Produção em Nova Janela (HTML) <<< NOVA FUNÇÃO
function exibirChecklistEmHTML(orcamento) {
    if (!orcamento) {
        console.error("exibirChecklistEmHTML: Orçamento inválido.");
        return;
    }
    console.log("Gerando checklist para orçamento:", orcamento);
    const janelaChecklist = window.open('./checklist.html', '_blank'); // Usa caminho relativo

    if (!janelaChecklist) {
        alert("Não foi possível abrir a janela de visualização do checklist. Verifique as permissões de pop-up do seu navegador.");
        return;
    }

    janelaChecklist.onload = () => {
        console.log("checklist.html carregado.");
        ...
        if (orcamento.produtos && orcamento.produtos.length > 0) {
            orcamento.produtos.forEach(produto => {
                // Gera um ID único para o checkbox e label, removendo caracteres inválidos
        ...
        // Optional: focus the checklist window
        // janelaChecklist.focus();
    };
...
function editarOrcamento(orcamentoId) {
    ...
    document.getElementById("prazoEntrega").value = orcamento.prazoEntrega || ''; // <<< PREENCHE PRAZO ENTREGA
...
async function atualizarOrcamento() {
    ...
    // Coleta os dados atualizados do formulário
      const orcamentoAtualizado = {
        ...
        total: converterMoedaParaNumero(document.getElementById("total")?.value),
        observacoes: document.getElementById("observacoes")?.value || '',
        prazoEntrega: document.getElementById("prazoEntrega")?.value || '', // <<< ATUALIZA PRAZO ENTREGA
    ...
    if (sucesso) {
        ...
        // Limpa campos textarea explicitamente
        const prazoEntregaField = document.getElementById("prazoEntrega");
        if (prazoEntregaField) prazoEntregaField.value = '';
        const observacoesField = document.getElementById("observacoes");
        if (observacoesField) observacoesField.value = '';
...
function editarPedido(pedidoId) {
    ...
    document.getElementById("prazoEntregaEdicao").value = pedido.prazoEntrega || ''; // <<< PREENCHE PRAZO ENTREGA EDICAO
...
async function atualizarPedido() {
    ...
    // Coleta dados atualizados do formulário de edição
     const pedidoAtualizado = {
        ...
        prazoEntrega: document.getElementById("prazoEntregaEdicao")?.value || '' // <<< ATUALIZA PRAZO ENTREGA EDICAO
        // tipo: 'pedido' // Definido em salvarDados
    };
    ...
    const sucesso = await salvarDados(pedidoAtualizado, 'pedido');
     ...
     if (sucesso) {
        ...
        pedidoEditando = null;
        // Opcional: Limpar formulário de edição se desejar (melhor não para não perder dados caso precise voltar)
...
document.addEventListener('DOMContentLoaded', () => {
        ...
        tabelaOrcamentos.addEventListener('click', (event) => {
            ...
            if (target.classList.contains('btnVisualizarOrcamento') && orcamentoId) {
                ...
                if (orcamento) {
                    ...
                    // Opcional: também mostrar checklist ao visualizar? Descomente abaixo se sim.
     ...
     if (formEdicaoPedido) {
        formEdicaoPedido.addEventListener('click', (event) => {
         ...
         formEdicaoPedido.addEventListener('input', (event) => {
             ...
             // Não precisa de listener para observacoesEdicao/prazoEntregaEdicao aqui, só na hora de salvar
         });
    ...
    // Inicializar campos de moeda no carregamento inicial (se não logado)
    // A função limparCamposMoeda também limpa os textareas agora
    if (!usuarioAtual) {
        limparCamposMoeda();
    }
