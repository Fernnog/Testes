...
let state = {
    ...
    // Prioridade 3: Melhoria de Arquitetura - Fila de Sincronização
    syncQueue: [],
    isSyncing: false,
};

// --- MELHORIA DE UX: Notificações Toast Não-Bloqueantes (Utilizando a versão do UI.js) ---
function showToast(message, type = 'success') {
    UI.showToast(message, type); // Delegado para o módulo de UI
}

// --- LÓGICA DE SINCRONIZAÇÃO COM GOOGLE DRIVE (Refatorada com Fila) ---
...
async function syncTarget(targetId) {
    ...
    try {
        ...
        console.error(`[App] Erro CRÍTICO ao sincronizar o alvo '${target.title}'.`, error);
...
/**
 * (Prioridade 3) Adiciona um alvo à fila de sincronização e inicia o processador.
 * @param {string} targetId - O ID do alvo a ser sincronizado.
 */
function requestSync(targetId) {
    if (!state.syncQueue.includes(targetId)) {
        state.syncQueue.push(targetId);
        console.log(`[Queue] Alvo ${targetId} adicionado à fila de sincronização.`);
    }
    processSyncQueue();
}

/**
 * (Prioridade 3) Processa a fila de sincronização um item por vez.
 */
async function processSyncQueue() {
    if (state.isSyncing || state.syncQueue.length === 0) {
        if (state.isSyncing === false && state.syncQueue.length === 0 && state.isDriveEnabled) {
            // Fila vazia e nada rodando, a sincronização terminou.
    ...
    // Prioridade 2: Atualiza a UI global para indicar atividade
    UI.updateDriveStatusUI('syncing', `Sincronizando ${state.syncQueue.length} item(s)...`);

    const targetId = state.syncQueue.shift(); // Pega o primeiro item da fila
    console.log(`[Queue] Processando alvo: ${targetId}. Restantes: ${state.syncQueue.length}`);

    try {
        await syncTarget(targetId);
    } catch (error) {
        console.error(`[Queue] Erro não capturado ao processar ${targetId}:`, error);
    } finally {
        state.isSyncing = false;
        // Chama a si mesma para processar o próximo item da fila
        setTimeout(processSyncQueue, 300); // Pequeno delay para não sobrecarregar a API
    }
}

/**
 * (Prioridade 1) Handler para forçar a sincronização de todos os alvos na carga inicial.
 * Agora utiliza o sistema de fila.
 */
function initialLoadSync() {
    if (!state.isDriveEnabled) return;

    console.log('[App] Adicionando todos os alvos à fila para sincronização de fundo...');

    const allTargetsToSync = [...state.prayerTargets, ...state.archivedTargets, ...state.resolvedTargets];
    
    // Limpa a fila atual e adiciona todos os alvos
    state.syncQueue = [];
    allTargetsToSync.forEach(target => {
        target.driveStatus = 'pending'; // Reseta o status visual
        delete target.driveErrorMessage;
        if (!state.syncQueue.includes(target.id)) {
            state.syncQueue.push(target.id);
        }
    });
    
    // Renderiza todos os painéis para mostrar o status 'pending'
...
// (Prioridade 1) Função de login simplificada
async function handleGoogleSignIn() {
    try {
        const { user } = await Auth.signInWithGoogle();
        if (user) {
            // O listener do `initializeAuth` já vai cuidar do resto (inicializar Drive, carregar dados).
            // Apenas damos um feedback imediato.
            showToast("Login com Google bem-sucedido!", "success");
        }
    } catch (error) {
        console.error("[App] Erro no fluxo de login com Google:", error);
...
function applyFiltersAndRender(panelId) {
    ...
    switch (panelId) {
        ...
        case 'archivedPanel': UI.renderArchivedTargets(paginatedData, filteredData.length, currentPage, targetsPerPage, state.dailyTargets); break;
...
async function loadDataForUser(user) {
    try {
        ...
        // Se o Drive estiver habilitado, enfileira todos os alvos para uma sincronização de fundo.
        initialLoadSync();
...
function handleLogoutState() {
    ...
    UI.renderTargets([], 0, 1, 10, {}); UI.renderArchivedTargets([], 0, 1, 10, {}); UI.renderResolvedTargets([], 0, 1, 10); UI.renderDailyTargets([], [], {}); UI.renderPriorityTargets([], {}); UI.resetPerseveranceUI(); UI.resetWeeklyChart(); UI.showPanel('authSection');
    // Prioridade 2: Atualiza a UI para desconectado no logout
...
async function handleAddNewTarget(event) {
    ...
    try {
        ...
        // Recarrega os dados para obter o ID do novo alvo
        const prayerData = await Service.fetchPrayerTargets(state.user.uid);
        state.prayerTargets = prayerData.map(t => ({ ...(state.prayerTargets.find(pt => pt.id === t.id) || {}), ...t, driveStatus: t.driveStatus || 'pending' }));

        const newTargetInState = state.prayerTargets.find(t => t.title === newTarget.title && !t.googleDocId);

        if (newTargetInState) {
             console.log(`[App] Enfileirando sincronização para o novo alvo ID: ${newTargetInState.id}`);
             requestSync(newTargetInState.id); // Adiciona à fila
        } else {
...
async function handlePray(targetId) {
    ...
    if (!targetToPray || state.dailyTargets.completed.some(t => t.id === targetId || t.targetId === targetId)) return;
    ...
    try {
        ...
        if (targetToPray.isPriority) {
            ...
            const allPriorityPrayed = priorityTargets.every(p =>
                state.dailyTargets.completed.some(c => c.id === p.id || c.targetId === p.id)
            );
        ...
        const completedIndex = state.dailyTargets.completed.findIndex(t => t.id === targetId || t.targetId === targetId);
        if (completedIndex > -1) {
...
// ===============================================
// Em script.js, substitua o listener de DOMContentLoaded existente por este bloco completo.

document.addEventListener('DOMContentLoaded', () => {
    // Lógica de inicialização principal com a correção de estado.
    Auth.initializeAuth(async (user) => {
        if (user) {
            // Passo 1: Atualiza a UI imediatamente para mostrar o status de "logado".
            UI.updateAuthUI(user);

            // Passo 2: Verifica se o provedor de login é o Google.
            const isGoogleUser = user.providerData.some(
                (provider) => provider.providerId === 'google.com'
            );

            if (isGoogleUser) {
                // Passo 3: Se for um usuário do Google, reexecuta o fluxo de login de forma silenciosa.
                // Isso obtém um novo token de acesso e reinicializa o serviço do Drive,
                // resolvendo o problema de estado ao recarregar a página.
                await handleGoogleSignIn();
                // A função loadDataForUser já é chamada dentro de handleGoogleSignIn (se bem-sucedido)
                // ou no próprio fluxo de inicialização do Auth, então não precisa ser chamada aqui novamente.
            } else {
                // Caso de borda: o usuário está logado, mas não com o Google.
                showToast("A sincronização com o Drive requer autenticação com o Google.", "error");
                state.isDriveEnabled = false;
                UI.updateDriveStatusUI('disconnected');
                await loadDataForUser(user); // Carrega os dados, mas sem a funcionalidade do Drive.
            }
        } else {
            // Se não houver usuário, reseta completamente o estado da aplicação.
    ...
    // NOVO LISTENER: Adicionado para o botão de sair na barra superior.
    document.getElementById('btnLogoutTop').addEventListener('click', () => Auth.handleSignOut());
    ...
    // --- DELEGAÇÃO DE EVENTOS CENTRALIZADA (Permanece inalterada) ---
    document.body.addEventListener('click', async e => {
        ...
        const { target, isArchived, panelId: resolvedPanelId } = findTargetInState(id);
        ...
        switch(action) {
            ...
            case 'add-new-observation':
                if (target) await handleAddObservation(target, isArchived, resolvedPanelId);
                break;
            case 'edit-title':
            ...
            case 'save-title': {
                ...
                applyFiltersAndRender(resolvedPanelId);
                try {
            ...
            case 'edit-category':
                ...
                break;
            case 'save-category':
                await handleSaveCategory(target, isArchived, resolvedPanelId);
                break;
